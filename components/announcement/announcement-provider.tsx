...
import { type AnnouncementConfig, type AnnouncementTrackingData, announcementService } from "./announcement-service"
import { AnnouncementModal } from "./announcement-modal"

type Ctx = {
  activeAnnouncement: AnnouncementConfig | null
  showAnnouncement: (announcement: AnnouncementConfig, forceShow?: boolean) => void
  dismissAnnouncement: (announcementId: string, permanent?: boolean) => void
  updateAnnouncementConfig: (announcement: Partial<AnnouncementConfig> & { id: string }) => void
  // Utilities
...
export function AnnouncementProvider({ children }: { children: React.ReactNode }) {
  const [activeAnnouncement, setActiveAnnouncement] = React.useState<AnnouncementConfig | null>(null)

  const showAnnouncement = React.useCallback((announcement: AnnouncementConfig, forceShow?: boolean) => {
    if (forceShow) {
      announcement.onShow?.()
    ...
    if (!announcementService.canShowAnnouncement(announcement)) {
      console.warn(`[Announcement] "${announcement.id}" cannot be shown due to conditions or limits`)
      return
    }
  ...
  const dismissAnnouncement = React.useCallback((announcementId: string, permanent?: boolean) => {
    announcementService.recordAnnouncementDismissed(announcementId, !!permanent)
    setActiveAnnouncement(null)
  }, [])

  const updateAnnouncementConfig = React.useCallback((announcement: Partial<AnnouncementConfig> & { id: string }) => {
    setActiveAnnouncement((prev) => {
  ...
  const handleOk = React.useCallback(
    async (id: string) => {
      ...
      if (shouldClose) {
        const markPermanent = (activeAnnouncement.onDismiss?.(true) ?? false) === true
  ...
  const handleCancel = React.useCallback(
    (id: string) => {
      if (!activeAnnouncement || activeAnnouncement.id !== id) return
      const markPermanent = (activeAnnouncement.onDismiss?.(false) ?? false) === true
      dismissAnnouncement(id, markPermanent)
    },
    [activeAnnouncement, dismissAnnouncement],
  )
  ...
  const value: Ctx = {
    ...
    resetAnnouncementTracking: (id) => announcementService.resetAnnouncementTracking(id),
  }
  ...
  return (
    <AnnouncementContext.Provider value={value}>
      {children}
      {activeAnnouncement ? (
        <AnnouncementModal config={activeAnnouncement} onOk={handleOk} onCancel={handleCancel} />
      ) : null}
...
export type { AnnouncementConfig, AnnouncementTrackingData } from "./announcement-service"
